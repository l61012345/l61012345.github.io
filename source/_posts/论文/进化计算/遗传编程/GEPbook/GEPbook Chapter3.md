...
但是不可否认的是，理论上由于GEP存在基因型和表现型的二重表示，如后文中所说，其表示结构中不仅存在中性块（语义结果为常数的区段），而且存在非编码区段。非编码区段是GEP中独有的一种表示设计，这个部分可以传播不表现的遗传信息，同时也可以让大面积的遗传突变有效化，尤其是后者，有效化的设计大幅度增强了GEP可以搜索的范围，因此相比于GP，在上述提出的模式当中有可能GEP的效果仍然更好一些。  
