...
<img src = https://cdn.jsdelivr.net/gh/l61012345/Pic/img/20240207133137.png width=20%>  
...
$$(F1^{σ1→σ2} ARG1^{σ1})^{σ2}$$
其中$ARG1$是函数$F1$的声明。  
使用科里化的表示方法的目的是为了更好的表示有多个声明的函数，如下面的函数$F2$拥有两个声明$ARG1$和$AGR2$，其类型分别为$σ1$和$σ2$，函数的返回类型为$σ3$，简单表示为：  
$$F2^{σ1→σ2→σ3}$$
展开表示为：  
