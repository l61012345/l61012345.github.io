...
$$ɛ_k=∑_iF_i(ω_j)P(i←j,k)=β_{jk}(ω_j)+\mathclose{R}(ω_j)$$
这个式子可以理解为对于个体$j$，添加一个block of code$k$之后fitness得到改进的概率。  
继续化简这个式子，**假设block of code对其附属的每一个个体的影响程度都是恒定的**，$ɛ_k$可以认为是这个block of code的contructional fitness.  
...
$$\frac{d}{dt}n_i(t)=\underset{exploration发生的概率和改进成功率的乘积}{αɛ_i}n_i(t)/N(t)$$
那么有两个block of code在程序中的扩散速率的比值呈现指数上升：  
$$\frac{n_i(t)}{n_j(t)}=\frac{n_i(0)}{n_j(0)}e^{α(ɛ_i-ɛ_j)∫_0^tdτ/N(τ)}$$
现在推广到整个个体，对于整个个体中所有的block of code，其平均contructional fitness可以表示为：  
$$\overline{ɛ}(t)=∑_iɛ_ip_i(t)$$
同样地对其进行求导：  
$$\frac{d}{dt}\overline{ɛ}(t)=\sum_iɛ_i\frac{d}{dt}(n_i(t)/N(t))=\frac{α}{N(t)}Var(ɛ)$$
其中$Var(ɛ)=[∑_iɛ_i^2p_i(t)-\overline{ɛ}(t)]$，表示contructional fitness的方差，代表多样性。那么根据上式，**某个个体的evolvability的增长速度与多样性成正比，与个体大小成反比**。  
